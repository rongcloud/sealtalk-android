...
public class IMInfoProvider {
    ...
    public void updateGroupInfo(String groupId) {
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            LiveData<Resource<GroupEntity>> groupSource =
                                    groupTask.getGroupInfo(groupId);
                            triggerLiveData.addSource(
                                    groupSource,
                                    resource -> {
                                        if (resource.status == Status.SUCCESS
     ...
     * @param callback
     */
    private void updateIMGroupMember(
            String groupId, RongMentionManager.IGroupMemberCallback callback) {
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            ...
                            triggerLiveData.addSource(
                                    ...
                                    resource -> {
                                        ...
                                        if (resource.status == Status.SUCCESS
                                            ...
                                            List<io.rong.imlib.model.UserInfo> userInfoList =
                                                    new ArrayList<>();
                                            for (GroupMember member : data) {
                                                ...
                                                io.rong.imlib.model.UserInfo info =
                                                        new io.rong.imlib.model.UserInfo(
                                                                member.getUserId(),
                                                                name,
                                                                Uri.parse(member.getPortraitUri()));
     ...
     */
    // todo
    //    private void updateCallGroupMember(String groupId, RongCallKit.OnGroupMembersResult
    // result) {
    ...
    //                }
    //
    //                if (resource.status == Status.SUCCESS && resource.data != null && result !=
    // null) {
    //                    List<GroupMember> data = resource.data;
    ...
    public void updateFriendInfo(String friendId) {
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            LiveData<Resource<FriendShipInfo>> friendInfo =
                                    friendTask.getFriendInfo(friendId);
                            triggerLiveData.addSource(
                                    friendInfo,
                                    resource -> {
                                        if (resource.status == Status.SUCCESS
     ...
     */
    public void getAllContactUserInfo(
            IContactCardInfoProvider.IContactCardInfoCallback contactInfoCallback) {
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            LiveData<Resource<List<FriendShipInfo>>> allFriends =
                                    friendTask.getAllFriends();
                            triggerLiveData.addSource(
                                    allFriends,
                                    resource -> {
                                        if (resource.status == Status.SUCCESS
                                            ...
                                            if (friendShipInfoList != null) {
                                                for (FriendShipInfo info : friendShipInfoList) {
                                                    ...
                                                    if (friendUser != null) {
                                                        io.rong.imlib.model.UserInfo user =
                                                                new io.rong.imlib.model.UserInfo(
                                                                        friendUser.getId(),
     ...
     * @param contactInfoCallback
     */
    public void getContactUserInfo(
            String userId, IContactCardInfoProvider.IContactCardInfoCallback contactInfoCallback) {
        ...
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            LiveData<Resource<FriendShipInfo>> friendInfo =
                                    friendTask.getFriendInfo(userId);
                            triggerLiveData.addSource(
                                    friendInfo,
                                    resource -> {
                                        if (resource.status == Status.SUCCESS
    ...
    public void refreshGroupNotideInfo() {
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            LiveData<Resource<List<GroupNoticeInfo>>> groupNoticeInfo =
                                    groupTask.getGroupNoticeInfo();
                            triggerLiveData.addSource(
                                    groupNoticeInfo,
                                    resource -> {
                                        if (resource.status == Status.SUCCESS
    ...
    public void refreshGroupExitedInfo(String groupId) {
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            LiveData<Resource<List<GroupExitedMemberInfo>>> groupExitedInfo =
                                    groupTask.getGroupExitedMemberInfo(groupId);
                            triggerLiveData.addSource(
                                    groupExitedInfo,
                                    resource -> {
                                        if (resource.status == Status.SUCCESS
    ...
    public io.rong.imlib.model.UserInfo getGroupMemberUserInfo(String groupId, String targetId) {
        GroupMember groupMember =
                dbManager.getGroupMemberDao().getGroupMemberInfoSync(groupId, targetId);
        io.rong.imlib.model.UserInfo userInfo = null;
        if (groupMember != null) {
            String groupMemberName =
                    TextUtils.isEmpty(groupMember.getGroupNickName())
                            ? groupMember.getName()
                            : groupMember.getGroupNickName();
            userInfo =
                    new io.rong.imlib.model.UserInfo(
    ...
    public void refreshReceivePokeMessageStatus() {
        ThreadManager.getInstance()
                .runOnUIThread(
                        () -> {
                            LiveData<Resource<GetPokeResult>> receivePokeMessageState =
                                    userTask.getReceivePokeMessageState();
                            triggerLiveData.addSource(
                                    receivePokeMessageState,
                                    resource -> {
