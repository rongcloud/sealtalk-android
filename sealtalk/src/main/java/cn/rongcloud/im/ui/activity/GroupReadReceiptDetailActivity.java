...
public class GroupReadReceiptDetailActivity extends TitleBaseActivity {
    ...
    private void updateReadGroupMemberList(List<GroupMember> groupMemberList) {
        ...
        // 获取消息中已读成员列表
        HashMap<String, Long> respondUserIdMap =
                targetMessage.getReadReceiptInfo().getRespondUserIdList();
        List<Map.Entry<String, Long>> respondUserIdList =
                new ArrayList<>(respondUserIdMap.entrySet());
        // 通过已读时间进行排序
        Collections.sort(
                respondUserIdList,
