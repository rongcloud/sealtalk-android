...
public class GRRDetailTestActivity extends TitleBaseActivity {
    ...
    private void initView() {
        ...
        sendTimeTv.setText(
                RongDateUtils.getConversationFormatDate(targetMessage.getSentTime(), this));
        ...
        msgExpandIv.setOnClickListener(
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (isMsgExpend == false) { // 未展开
                            isMsgExpend = true;
                            msgExpandIv.setImageDrawable(
                                    getResources()
                                            .getDrawable(
                                                    R.drawable
                                                            .conversation_ic_read_receipt_up_arrow));
                            ...
                            msgExpandIv.setImageDrawable(
                                    getResources()
                                            .getDrawable(
                                                    R.drawable
                                                            .conversation_ic_read_receipt_down_arrow));
        ...
        sendMsgTv.post(
                new Runnable() {
                    @Override
                    public void run() {
                        msgExpandIv.setVisibility(
                                sendMsgTv.getLineCount() > DEFAULT_MESSAGE_SHOW_LINE
                                        ? View.VISIBLE
                                        : View.GONE);
                        sendMsgTv.setMaxLines(DEFAULT_MESSAGE_SHOW_LINE);
                    }
        ...
        readTabTextTv.setText(
                "已读"); // getString(R.string.seal_conversation_read_receipt_read_persons_format, 0)
        ...
        View unreadView =
                LayoutInflater.from(this)
                        .inflate(R.layout.conversation_layout_read_receipt_member, null);
        ...
        // 成员点击事件
        RlGroupMemberAdapter.OnItemClickedListener memberClickedListener =
                new RlGroupMemberAdapter.OnItemClickedListener() {
                    @Override
        ...
        // 设置页面切换监听
        memberViewPager.addOnPageChangeListener(
    ...
    private void setTabSelectedBg(TextView line, TextView text, boolean isSelected) {
        if (isSelected) {
            ...
            line.setHeight(6);
            text.setTextColor(
                    getResources().getColor(R.color.color_read_receip_detail_tab_selected));
        } else {
            line.setBackgroundColor(getResources().getColor(R.color.color_transparent));
            line.setHeight(1);
            text.setTextColor(
                    getResources().getColor(R.color.color_read_receip_detail_tab_unselected));
        }
    }
                                ...
                                if (resource.data != null) {
                                    ReadReceiptV2Manager.getInstance()
                                            .getGroupMessageReaderList(
                                                    targetMessage,
                                                    new ReadReceiptV2Manager
                                                            .IGetMessageReadUserListCallback() {
                                                        @Override
                                                        public void onSuccess(
                                                                int members,
                                                                List<GroupMessageReader> readers) {
    ...
    private void updateReadGroupMemberList(List<GroupMember> groupMemberList) {
        ...
        // 获取消息中已读成员列表
        List<GroupMessageReader> respondUsers =
                targetMessage.getGroupReadReceiptInfoV2().getRespondUserIdList();
        //        List<Map.Entry<String, Long>> respondUserIdList = new
        // ArrayList<>(respondUserIdMap.entrySet());
        // 通过已读时间进行排序
        ...
        Collections.sort(
                respondUsers,
                new Comparator<GroupMessageReader>() {
                    @Override
    ...
    private void showMemberInfo(GroupMember groupMember) {
        ...
        Group groupInfo =
                RongUserInfoManager.getInstance().getGroupInfo(targetMessage.getTargetId());
        if (groupInfo != null) {
