 ...
 * 全局错误码枚举
 *
 * <p>因为目前不同的 API 会根据业务情况返回相同的错误码，所以需要根据 API 区分每个错误码的提示，所以做出以下处理。
 *
 * <p>API 类型的错误码规则： 每个 API 的错误码由 API 的 url 对应的代码{@link cn.rongcloud.im.net.SealTalkUrlCode} + API
 * 对应的错误码 比如登录接口 LOGIN，此 API 的 url 代码为 1，当 API 返回错误码 为 1000 且错误码偏移为 10000 时,对应 ErrorCode 为 11000
 */
public enum ErrorCode {
    ...
    USERNAME_OR_PASSWORD_INVALID(
            SealTalkUrlCode.LOGIN + 1001, R.string.seal_login_toast_phone_or_psw_error),
    API_SEND_CODE_OVER_LIMIT(
    ...
    REGISTER_PHONE_ALREADY_EXISTED(
            SealTalkUrlCode.REGISTER + 400, R.string.login_register_phone_already_existed),
    ...
    NONE_ERROR(-1, 0),
    GROUP_IN_PROTECT(
            SealTalkUrlCode.GROUP_COPY + 20004, R.string.seal_group_manager_copy_in_protect),
    GROUP_COPYED_IN_SEVEN_DAYS(
            SealTalkUrlCode.GROUP_COPY + 20005, R.string.seal_group_manager_copy_copied_7),
