...
/**
 * 配合 {@link CommonListBaseFragment} 使用。 此类定义了基础的数据获取和监听熟悉的逻辑规则。
 *
 * <p>此类和 {@link CommonListBaseFragment}、{@link CommonListAdapter} 相互配合使用。以达到快速的实现自定义数据的列表功能展示。
 *
 * <p>{@link #loadData()} 为数据记载方法。 此方法会在{@link cn.rongcloud.im.ui.fragment.ListBaseFragment} 中
 * 主动调用。继承并实现 {@link #loadData()} 方法。 在 loadData（） 方法中实现数据的获取逻辑， 即可实现数据自定义。
 *
 * <p>加入您获取数据源时需要监听 LiveData ， 则您可使用 <code> conversationLiveData </code> 变量进行一下操作。 <code>
 ...
 * </code>
 *
 * <p>通过上面的代码的方式可进对原数据原进行监听并处理。
 *
 * <p>此类中的数据类型为 {@link ListItemModel} 。 是为了适配多种数据源。
 *
 * <p>当获取到原数据后， 可使用 {@link ModelBuilder} 类进行对数据添加、转换、添加首字母以及通知下发数据的处理。 <code>
 * <p>
 ...
 * </code>
 *
 * <p>通过上面代码即可进行数据处理和数据下发展示。
 *
 * <p>此类中已经兼容 {@link FriendShipInfo} 和 {@link GroupEntity} 两个类型的默认转换。 只需调用
 *
 * <p>ModelBuilder builder = new ModelBuilder(); builder.addFriendList(List<FriendShipInfo>) 或
 * builder.addGroupList(List<GroupEntity) 等方法进行设置即可。
 *
 * <p>同时也支持其他类型自己进行封装。 类中也通过了其他类型转换的成 ListItemModel 的方法。 {@link #createGroupModel(GroupEntity)}
 * {@link #createFriendModel(FriendShipInfo)} {@link #createFunModel(FunctionInfo)} 或 {@link
 * #createFunModel(String, String)} {@link #createTextModel(String)}
 *
 * <p>上面这四种类型类中已经默认实现了。 当需要丰富或更改填充数据时， 可在子类中进行复写即可， 如下 <code>
 ...
 * </code>
 *
 * <p>也可单独实现自己进行调用。 如果有其他的新加类型， 则可使用自己进行封装转换成 ListItemModel。然后使用 builder.addModelList(model) 或
 * builder.addMode（model）进行添加。
 *
 * <p>ListItemModel 封装转换需注意的是， 必须要添加 id，name ， itemView 字段。 如果想在列表中显示复选按钮， 则需要设置 CheckStatus 字段不为
 * Node。
 *
 * <p>builder.buildFirstChar(); 方法是把当前设置好的集合生成首字母。用于 Side 的侧栏查询。
 *
...
public abstract class CommonListBaseViewModel extends AppViewModel {
    ...
    protected ListItemModel createGroupModel(GroupEntity entity) {
        ListItemModel.ItemView itemView =
                new ListItemModel.ItemView(
                        R.layout.item_common_group,
                        ListItemModel.ItemView.Type.GROUP,
                        CommonGroupItemViewHolder.class);
        ListItemModel<GroupEntity> model =
                new ListItemModel<>(entity.getId(), entity.getName(), entity, itemView);
    ...
    protected ListItemModel createFriendModel(FriendShipInfo info) {
        String name =
                TextUtils.isEmpty(info.getDisplayName())
                        ? info.getUser().getNickname()
                        : info.getDisplayName();
        ListItemModel.ItemView itemView =
                new ListItemModel.ItemView(
                        R.layout.item_common_conversation,
                        ListItemModel.ItemView.Type.FRIEND,
                        CommonFriendItemViewHolder.class);
        ListItemModel<FriendShipInfo> model =
                new ListItemModel<>(info.getUser().getId(), name, info, itemView);
    ...
    protected ListItemModel createTextModel(String name) {
        ListItemModel.ItemView itemView =
                new ListItemModel.ItemView(
                        R.layout.item_common_text,
                        ListItemModel.ItemView.Type.TEXT,
                        CommonTextItemViewHolder.class);
    ...
    protected ListItemModel createFunModel(String funId, String funContent) {
        ListItemModel.ItemView itemView =
                new ListItemModel.ItemView(
                        R.layout.item_common_fun,
                        ListItemModel.ItemView.Type.FUN,
                        CommonFunItemViewHolder.class);
        ListItemModel model =
                new ListItemModel(funId, funContent, new FunctionInfo(funId, funContent), itemView);
        return model;
    }
    ...
    /** 加载数据， 在此方法中实现加载数据的请求， 并调用 convertAndPost 方法转换成展示类型并 LiveData 通知出去 */
    ...
    /**
     * 这里自动去添加 view 和 ViewHolder 的映射关系， 用于在 {@link CommonListAdapter} 中进行自动创建View 对应的 ViewHolder
     *
     * @param out
     */
    private void initViewHzolder(List<ListItemModel> out) {
        for (ListItemModel model : out) {
            ...
            ViewHolderFactory.getInstance()
                    .putViewHolder(itemView.getItemResId(), itemView.getViewHolder());
        }
    }

    /**
     * 数据构建对象。 通过对象可添加数据并转成展示数据类型。 通过 ModelBuilder#post（） 方法则可通知数据下发。
     *
     * @return
     */
    ...
    /**
     * 此类用于在获取数据源后对数据源进行封装并下发。 当前支持 FriendShipInfo 和 GroupEntity 类型的装换。 其他类型可自己进行转换。 其中还有对以添加数据进行
     * Side 列表的排序构造的 {@link #buildFirstChar()} 方法。 处理添加完数据后， 调用 {@link #post()} 方法即可。
     */
    public class ModelBuilder {
        ...
        public void post() {
            /**
             * 这里自动去添加 view 和 ViewHolder 的映射关系， 用于在 {@link CommonListAdapter} 中进行自动创建View 对应的
             * ViewHolder
             */
            initViewHzolder(out);
