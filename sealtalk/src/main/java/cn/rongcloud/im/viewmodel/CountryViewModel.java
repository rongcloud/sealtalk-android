...
public class CountryViewModel extends AndroidViewModel {
    ...
    private SingleSourceMapLiveData<Resource<List<RegionResult>>, Resource<List<CountryInfo>>>
            countryInfoList;
    ...
    public CountryViewModel(@NonNull Application application) {
        ...
        countryInfoList =
                new SingleSourceMapLiveData<>(
                        new Function<Resource<List<RegionResult>>, Resource<List<CountryInfo>>>() {
                            @Override
                            public Resource<List<CountryInfo>> apply(
                                    Resource<List<RegionResult>> input) {
                                ...
                                if (regionList != null) {
                                    for (RegionResult region : regionList) {
                                        ...
                                        Collections.sort(
                                                countryInfos,
                                                new Comparator<CountryInfo>() {
                                                    @Override
                                                    public int compare(
                                                            CountryInfo lhs, CountryInfo rhs) {
                                                        return lhs.getFirstChar()
                                                                .compareTo(rhs.getFirstChar());
                                                    }
                                                });
                                    }
                                }

                                Resource<List<CountryInfo>> resource =
                                        new Resource<>(input.status, countryInfos, input.code);
                                return resource;
                            }
                        });
    }
    ...
    private LangUtils.RCLocale local(Context context) {
        ...
        if (LangUtils.RCLocale.LOCALE_CHINA == appLocale
            ...
            //            Locale systemLocale =
            // RongConfigurationManager.getInstance().getSystemLocale();
            //            if (systemLocale.getLanguage().equals(Locale.CHINESE.getLanguage())) {
            //
            // RongConfigurationManager.getInstance().switchLocale(LangUtils.RCLocale.LOCALE_CHINA,
            // context);
            //                return LangUtils.RCLocale.LOCALE_CHINA;
            //            } else {
            //
            // RongConfigurationManager.getInstance().switchLocale(LangUtils.RCLocale.LOCALE_US,
            // context);
    ...
    public void loadCountryDatas(String filterStr) {
        if (TextUtils.isEmpty(filterStr)) {
            if (countryInfoList.getValue() == null
                    || countryInfoList.getValue().status != Status.SUCCESS) {
                filterCountryList.removeSource(countryInfoList);
                filterCountryList.addSource(
                        countryInfoList,
            ...
            if (infos != null) {
                ...
                for (CountryInfo info : infos) {
                    ...
                    if (name.indexOf(filterStr) != -1
                            || characterParser.getSpelling(name).startsWith(filterStr)) {
                ...
                Resource<List<CountryInfo>> resource =
                        new Resource<>(
                                countryInfoList.getValue().status,
                                countryInfos,
                                countryInfoList.getValue().code);
